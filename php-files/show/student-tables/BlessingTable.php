...
$queryForActiveBlessingTableStudents = "SELECT * FROM Students JOIN Student_To_Programs ON Students.Id = Student_To_Programs.Student_Id 
                                WHERE Student_To_Programs.Program_Id = 3 && Students.Active_Student = 1;";
        ...
        <form method="POST" action="#" name="inactiveStudentsTable"
            ...
            <div class="table-responsive">
                <table id="search-table" class="table table-striped table-condensed table-hover">
                    ...
                    <tbody>
                    <?php
                    while ($blessingTableRow = mysqli_fetch_array($blessingTableResults, MYSQLI_ASSOC)) {
                        $numberOfProgramsRow = mysqli_fetch_array($enrolledProgramResults, MYSQLI_ASSOC);
                        $studentDocumentFlag = false;
                        $dynamicRowId++;
                        $studentIdToSearch = $blessingTableRow['Id'];
                        $studentName = $blessingTableRow['First_Name'] . " " . $blessingTableRow['Last_Name'];
                        ...
                        <tr class='number-row'>
                            ...
                            <?php
                            if (($blessingTableRow['Permission_Slip'] == 1)) { ?>
