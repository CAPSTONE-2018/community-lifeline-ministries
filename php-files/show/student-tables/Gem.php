...
$queryForActiveGemStudents = "SELECT DISTINCT * FROM Students JOIN Student_To_Programs ON Students.Id = Student_To_Programs.Student_Id 
                                WHERE Student_To_Programs.Program_Id = 2 && Students.Active_Student = 1;";
        ...
        <form method="POST" action="#" name="inactiveStudentsTable"
            ...
            <div class="table-responsive">
                <table id="search-table" class="table table-striped table-condensed table-hover">
                    ...
                    <tbody>
                    <?php
                    while ($gemRow = mysqli_fetch_array($gemResults, MYSQLI_ASSOC)) {
                        $numberOfProgramsRow = mysqli_fetch_array($enrolledProgramResults, MYSQLI_ASSOC);
                        $studentDocumentFlag = false;
                        $dynamicRowId++;
                        $studentIdToSearch = $gemRow['Id'];
                        $studentName = $gemRow['First_Name'] . " " . $gemRow['Last_Name'];
